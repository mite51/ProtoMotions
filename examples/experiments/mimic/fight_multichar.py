...
must resume from a Tier-4 (or earlier fight-tier) checkpoint.
...
What Tier 5 adds on top of the frozen base:
  ...
  - ``opponent_impact`` reward: rewards landing fast hand/foot strikes on an
    opponent (closing speed gated by proximity). The Tier-2 dangerous-impact
    penalty already covers being hit by anything (including an opponent's limbs).
...
Train (resume/warm-start from the stamina tier):
    python protomotions/train_agent.py \
        ...
        --motion-file <motions.pt> --num-envs 4096 --batch-size 8192 \
