...
class Simulator(RecordingMixin, ABC):
    ...
    def _throw_projectile(self, env_ids: Optional[torch.Tensor] = None) -> None:
        ...
        if env_ids.numel() == 0:
            return
        num_e = env_ids.numel()
        if not cfg.auto_throw_enabled:
            # Preserve main's round-robin behavior for manual viewer throws.
            cube_idx = self._proj_next_idx[env_ids].clone()
        else:
            throw_time = self._proj_throw_time[env_ids]
            free = ~torch.isfinite(throw_time)
            score = torch.rand(num_e, cfg.num_projectiles, device=self.device) + 2.0 * free
