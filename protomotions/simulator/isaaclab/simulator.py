...
class IsaacLabSimulator(Simulator):
    ...
    def _apply_simulator_pd_targets(self, pd_targets: torch.Tensor) -> None:
        ...
        for c in range(self.num_characters):
            self._robots[c].set_joint_position_target(targets[:, c], joint_ids=None)
    ...
    def _apply_simulator_torques(self, torques: torch.Tensor) -> None:
        ...
        for c in range(self.num_characters):
            self._robots[c].set_joint_effort_target(t[:, c], joint_ids=None)

    def set_joint_gain_scale(
        self, scale: torch.Tensor, env_ids: Optional[torch.Tensor] = None
    ) -> None:
        """Write per-env, per-DOF PD gains into PhysX (BUILT_IN_PD stamina path).

        ``scale`` is in common DOF ordering, shape ``[len(env_ids), num_actions]``
        (or ``[num_envs, num_actions]`` when ``env_ids`` is None). Effective gains
        are ``nominal * scale`` for both stiffness and damping. Values are
        permuted to the simulator joint order and written per articulation via
        Isaac Lab's ``write_joint_stiffness/damping_to_sim`` (which set PhysX DOF
        properties directly and persist across steps/resets). The implicit
        actuator caches are also synced so torque bookkeeping matches.
        """
        if env_ids is None:
        ...
        # nominal (common order) -> effective (common order) -> sim order.
        stiffness = self._common_p_gains.unsqueeze(0) * scale
        damping = self._common_d_gains.unsqueeze(0) * scale
        conv = self.data_conversion.dof_convert_to_sim
        stiffness_sim = stiffness[:, conv]
        damping_sim = damping[:, conv]

        if self.num_characters == 1:
            self._write_joint_gains(self._robot, stiffness_sim, damping_sim, env_ids)
            return

        # env_ids are flattened RL rows; route each character's gains to its
        # articulation, writing only the physical scenes involved.
    ...
    @staticmethod
    def _write_joint_gains(
        robot,
        stiffness_sim: torch.Tensor,
        damping_sim: torch.Tensor,
        phys_env_ids: torch.Tensor,
    ) -> None:
        """Write full-joint stiffness/damping for ``phys_env_ids`` of one articulation."""
        robot.write_joint_stiffness_to_sim(
            stiffness_sim, joint_ids=None, env_ids=phys_env_ids
        )
        robot.write_joint_damping_to_sim(
            damping_sim, joint_ids=None, env_ids=phys_env_ids
        )
        # Implicit actuators do not re-push gains during stepping, but they keep a
        # cached copy used to report applied torque; sync it so power/effort
        # bookkeeping reflects the runtime gains (Isaac Lab issue #128/#1552).
        # ``actuator.stiffness`` is [num_envs, num_joints_in_actuator] indexed by the
        # actuator's LOCAL columns, while ``joint_indices`` are GLOBAL sim indices
        # (used to gather from the full-width ``*_sim`` tensors). The columns line up
        # 1:1 in order, so assign all local columns from the globally-gathered slice.
        for actuator in robot.actuators.values():
            gj = actuator.joint_indices
            if not isinstance(gj, slice):
                gj = torch.as_tensor(gj, device=stiffness_sim.device, dtype=torch.long)
            actuator.stiffness[phys_env_ids] = stiffness_sim[:, gj]
            actuator.damping[phys_env_ids] = damping_sim[:, gj]
    ...
    def get_robot_body_masses(self) -> torch.Tensor:
        ...
        try:
            per_robot = [
                robot.root_physx_view.get_masses() for robot in self._robots
            ]
    ...
    def _get_simulator_bodies_state(
        ...
        isaacsim_bodies_positions = self._stack_robots(
            [r.data.body_pos_w.view(E, self._num_bodies, 3) for r in self._robots]
        )
        isaacsim_bodies_rotations = self._stack_robots(
            [r.data.body_quat_w.view(E, self._num_bodies, 4) for r in self._robots]
        )
        isaacsim_bodies_velocities = self._stack_robots(
            [r.data.body_lin_vel_w.view(E, self._num_bodies, 3) for r in self._robots]
        )
        isaacsim_bodies_ang_velocities = self._stack_robots(
            [r.data.body_ang_vel_w.view(E, self._num_bodies, 3) for r in self._robots]
        )
        ...
        for c in range(self.num_characters):
            ...
            for body_idx, body_name in enumerate(sim_body_names):
                if body_name in sensor_map:
                    ...
                    forces[:, body_idx, :] = contact_sensor.data.net_forces_w.clone()[
                        :, 0, :
                    ]
