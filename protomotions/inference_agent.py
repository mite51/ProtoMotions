...
Example::
    ...
    PYTHON_PATH protomotions/inference_agent.py --simulator isaaclab --num-envs 16 \\
        ...
        --throw --auto-throw-prob 0.05 --randomize-stamina --stamina-range 0.75 1.0
...
def create_parser():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "1.0 == nominal strength; start with 0.75 1.0."
        ),
    )
...
def main():
    ...
    if args.throw or args.auto_throw_prob is not None:
        ...
        if simulator_config.projectile is None or simulator_config.projectile.num_projectiles == 0:
            ...
            simulator_config.projectile = ProjectileConfig(
                speed_range=(12.0, 22.0), density=300.0, num_projectiles=3,
